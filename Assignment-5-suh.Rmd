...
- Mann-Whitney U (non paired) or wilcoxon signed rank (paired) ??
...
mwu_salaries_p <- wilcox.test(med_sal$p_sal_m, med_sal$p_sal_f) 
...
mwu_salaries_np <- wilcox.test(med_sal$np_sal_m, med_sal$np_sal_f) 

mwu_salaries_np
# p = 1.886e-05
# this tells us to reject null. There is no significant diff

# Cliffs delta
np_sal_delta <- cliff.delta(med_sal$np_sal_m, med_sal$np_sal_f)
np_sal_delta
# effect size is 0.92 (LARGE)
...
# AIC for first model is smaller, do we want to keep that though?? Judgement says we should not have years_since and years_service in the same model. 

```

```{r stargazer, results = 'asis'}
stargazer(test_lm1, test_lm2, type = "html")
```
