...
```{r}
med_sal <- read_csv("Median salary for doctoral recipients.csv")

med_sal_p_m <- filter(med_sal, employment_type == "p")

med_sal_np <- filter(med_sal, employment_type == "np")


## No idea how to do this one

wsr_salaries <- wilcox.test(med_sal_p$median_salary, med_sal_p$median_salary, paired = TRUE)
