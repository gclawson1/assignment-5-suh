...
What if we want to describe the way that one variable
changes with respect to another variable?
Then we might use regression to:
(1) Understand how well input (x) variable(s) predict the
value of an outcome (y) variable
(2) How do input variables influence the output value?
(3) Find a regression equation that *MAY* be used to
make predictions for currently unknown values
...
(1) Look at and think about your data
Does a linear relationship make the most sense, conceptually & visually?
(2) Run the regression
lm(y ~ x, data = df)
(3) Check the diagnostic plots
plot(model_name) – for evidence of homoscedasticity and normal residuals
(4) Communicate the results in text/table/figure  
...
# there is a significant association between year, department, and number of phds awarded. 

# females moved out of education, and into the sciences and engineering. The humanities generally stayed the same. 
```
...
```{r}
med_sal <- read_csv("Median salary for doctoral recipients cleaned.csv")  # reading in NA values that aren't there!!!!

wsr_salaries_p <- wilcox.test(med_sal$p_sal_m, med_sal$p_sal_f)
...
H0: The model does not significantly predict the
outcome (i.e., there is no relationship between the
model and the outcome; no variables significantly
predict outcome)
HA: AT LEAST ONE of the explanatory variables
significantly predicts the outcome 


How do I know which model is the BEST
one?
1. STATISTICS IS NO SUBSTITUTE FOR
JUDGMENT.
2. ASSUMPTIONS.
...
coefficient
• Large	changes	in	model	output	for	small	
changes	in	predictor	variable	inputs
• Illogical/unexpected	 coefficients	 (esp.	
coefficient	sign)

Variance	Inflation	Factor
How	much	the	variance	in	coefficients	is	inflated	
(compared	 to	the	variance	if	there	were	no	
correlation	 between	model	variables)	
VIF	=	1:	No	correlation	(variance	is	not	inflated	at	all	by	correlation	
with	other	predictor	variables)
VIF	>	4:	Eeehhh…some	correlation,	investigate	further	and	think	
critically	about	possible	overlaps	in	explanatory	variables
VIF	>	10:	Serious	correlation	between	variables	that	may	be	
strongly	inflating	coefficient	variance	and	error
...
plot(faculty_salary$years_since, faculty_salary$salary) 
plot(faculty_salary$years_service, faculty_salary$salary)
plot(faculty_salary$years_since, faculty_salary$years_service) # colinear
